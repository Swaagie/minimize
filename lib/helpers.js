...
Helpers.prototype.text = function text (element, data) {
  ...
  // Collapse space between text and inline elements, clobber space without inline elements.
  if (element.prev && this.isInline(element.prev))
    text = text.replace(/^\s+/, ' ');
  else
    text = text.replace(/^\s+/, '');

  if (element.next && this.isInline(element.next))
    text = text.replace(/\s+$/, ' ');
  else
    text = text.replace(/\s+$/, '');

  // If we have ancestors stored do not remove structure.
  if (!ancestors) text = text.replace(/\n/g, ' ').replace(/\s+/g, ' ');

  // Remove CDATA from scripts.
  if (!this.config.cdata && ancestors && this.isJS(this.ancestor[ancestors - 1])) {
    text = text.replace(cdata.start, '').replace(cdata.end, '');
  }
