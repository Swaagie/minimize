 ...
 *
 * @Constructor
 * @param {Mixed} parser HTMLParser2 instance, i.e. to support SVG if required.
 ...
 * @api public
 */
Minimize.prototype.parse = function parse(content, callback, sync) {
  var id = uuid.v4();
...
/**
 * [parseSync description]
 * @param  {[type]} content [description]
 * @return {[type]}         [description]
 */
Minimize.prototype.parseSync = function parseSync(content) {
  var output;

  this.parse(content, function (error, result) {
...
Minimize.prototype.minifier = function minifier(id, sync, error, dom) {
  ...
  //
  return this[!sync ? 'traverse' : 'traverseSync'](dom, '', this.emits('parsed:' + id));
};

Minimize.prototype.traverseSync = function traverseSync(data, html, done) {
  console.log('SKDJHKSHDJKSHd')
  done(null, html + 'lol');
};
