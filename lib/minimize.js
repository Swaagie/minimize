...
Minimize.prototype.traverse = function traverse(data, html, done) {
  ...
  async.reduce(data, html, function reduce(html, element, step) {
    ...
    //
    if (!plugins.length) return open();
    async.eachSeries(plugins, function plug(plugin, next) {
      minimize.plugins[plugin].call(minimize, element, next);
    }, open);

    /**
     * Add opening element to minified HTML.
     */
    function open(error) {
    ...
    /**
     * Add closing element to minified HTML.
     *
     * @param {Error} error
     * @param {String} html
     * @api private
